...
public class ProdottoController {
	...
	/**
	 * @GetMapping("/getmodel") public ResponseEntity<Prodotto> getmodel() {
	 * Prodotto prodott = new Prodotto(); return new
	 * ResponseEntity<Prodotto>(prodott, HttpStatus.CREATED); }
	 */

	/**
	 * @PostMapping("/saveupdate") public ResponseEntity<Prodotto>
	 * saveOrUpdateProdotto(@RequestBody Prodotto prodotto) { try { Prodotto saved =
	 * prodottoService.saveOrUpdateProdotto(prodotto); logger.info(saved + "
	 * saved"); return new ResponseEntity<Prodotto>(saved, HttpStatus.CREATED); }
	 * catch (Exception e) { logger.error("Errore " + e); return new
	 * ResponseEntity<Prodotto>(HttpStatus.INTERNAL_SERVER_ERROR); } }
	 */
	...
	private Object prodotto(List<Prodotto> prodotti) {
